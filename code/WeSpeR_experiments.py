...
# from WeSpeR_HD import WeSpeR_HD_minimization
...
def WeSpeR_experiment_1(tau_init = None, p = 2000, n = 20000):
    benchmark = True
    quest = True
    plots = False
    ...
    if quest:
        print("WeSpeR:", quest) 
        start_time = time.time()
        model, tau_fit, all_tau = WeSpeR_MD_minimization(torch.tensor(lambda_), tau_init, wt = None, d = d, wd = wd, c = c, mu = mu, weights = weights, w_args = w_args, omega = omega, lr = lr, n_epochs = n_epochs_q, decay = decay, loss_type = loss_type, method = method, verbose = verbose)
        last_time = time.time()
        ...
        # plt.title("PRIAL S train WeSpeR")
        # plt.legend()
        # plt.show()
        ...
        # plt.title("PRIAL P train WeSpeR")
        # plt.legend()
        # plt.show()
    ...
    if benchmark:
        ...
        start_time = time.time()
        model, tau_fit, bench_loss, all_tau = WeSpeR_LD_minimization(torch.tensor(S), n, p, tau_init, torch.sqrt(W), method = "Adam", n_epochs = n_epochs_b, b = b, assume_centered = False, lr = 5e-2, momentum=0., verbose = verbose)
        last_time = time.time()
        print("Compute time:", last_time - start_time, "s.")
...
def WeSpeR_experiment_4(tau_init = None, p = 2000, n = 20000):
    LD = True
    MD = True
    plots = False
    ...
    if LD:
        print("WeSpeR_LD:", LD)
        start_time = time.time()
        estimator_LD = WeSpeR_LD(bias=True, assume_centered=True).fit_from_S(S, W, p_tau = p_tau, tau_init = tau_init, method = 'Adam', n_epochs = n_epochs_LD, b = b, assume_centered = True, lr = lr, momentum=0., verbose = True)
        last_time = time.time()
        print("Compute time:", last_time - start_time, "s.")
...
if __name__ == '__main__':
    ...
    #         print("p =", p)
    #         _ = WeSpeR_experiment_1(tau_init = None, p = p, n = 10*p)
    
    # tau_fit = None
    # model, tau_fit, tau_init, f, F, xi, f_i, F_i, xi_i, lambda_ = WeSpeR_experiment_1(tau_init = None, p = 10000, n = 100000)

    # tau_fit = None
    # for p in [50000]:
    #     print("p =", p)
    #     lambda_, _, _, _, _, _ = WeSpeR_experiment_2(tau_init = None, p = p, n = 2*p)

        
    # tau_fit = None
    # model, tau_fit, tau_init, f, F, xi, f_i, F_i, xi_i, lambda_ = WeSpeR_experiment_3(H_distrib = 1, tau_init = tau_fit, p = 1000, n = 3000)
    
    # tau_fit = None
    # model, tau_fit, tau_init, f, F, xi, f_i, F_i, xi_i, lambda_ = WeSpeR_experiment_3(H_distrib = 2, tau_init = tau_fit, p = 1000, n = 3000)
    
    # tau_fit = None
    # model, tau_fit, tau_init, f, F, xi, f_i, F_i, xi_i, lambda_ = WeSpeR_experiment_3(H_distrib = 3, tau_init = tau_fit, p = 1000, n = 3000)
    
    # tau_fit = None
    # model, tau_fit, tau_init, f, F, xi, f_i, F_i, xi_i, lambda_ = WeSpeR_experiment_3(H_distrib = 4, tau_init = tau_fit, p = 1000, n = 3000)
    ...
    estimator_LD, estimator_MD, tau_init, S =  WeSpeR_experiment_4(tau_init = None, p = 100, n = 1000)